...
def prediction(beta_0, beta_1, X):
    ...
    y_pred = beta_0 + beta_1 * X # y 절편 + (회귀계수 * 입력값)
...
def gradient_descent(X, y, iters, lr):
    ...
    beta_0 = np.zeros((1,1)) #튜플을 만듬, 값을 기록하려고
    print(beta_0)
    beta_1 = np.zeros((1,1))
    print(beta_1)
    ...
    for i in range(iters):
        ...
        loss = np.mean(np.square(y - y_pred)) #오차 MSE
...
def plotting_graph(X,y,beta_0,beta_1):
    ...
    plt.savefig("./ml/ML3/q1_test.png")
...
def main():
    ...
    # 학습을 위해 필요한 파라미터입니다. 
    lr = 1e-4 # 얼마나 어떤 방향으로 업데이트를 할 것인가? (머신러닝, 딥러닝하고 다름) 학습용..
    iteration = 1000
