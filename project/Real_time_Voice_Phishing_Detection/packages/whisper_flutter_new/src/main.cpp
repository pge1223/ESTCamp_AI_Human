...
static struct whisper_context *cached_ctx = nullptr;
...
json transcribe(json jsonBody) noexcept
{
    ...
    // whisper init — model loaded once, reused across calls
    if (cached_ctx == nullptr || cached_model_path != params.model) {
        if (cached_ctx != nullptr) whisper_free(cached_ctx);
        cached_ctx = whisper_init_from_file(params.model.c_str());
        cached_model_path = params.model;
    }
    struct whisper_context *ctx = cached_ctx;
